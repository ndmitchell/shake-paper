...
main = shake shakeOptions $ do
    ...
    rule $ \(ConfigKey key) -> Just $ do
        [mp] <- apply [ConfigMap "config.file"]
        return $ mp Map.! key
...
parseConfig = ...
\end{lstlisting}
\noindent We define two new types of rule. We define a rule from configuration
file to the key/value pairs it contains, using the \lst'ConfigMap' type. Next we
define a rule from a single key in the configuration file to the associated value,
...
Crucially, Shake treats these computation incrementally: it parses each
...
The use of non-file rule types is common in large build systems, we use ten
different types in our implementation of GHC's build system in \S\ref{sec:ghc}.
