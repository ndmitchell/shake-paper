...
the entire dependency graph rather that a series of framgments.
...
In this paper we substante this claim, and offer a solution,
...
\make{} and similar build systems (Sections~\ref{sec:challenges} and
\ref{sec:solutions}.  Over the last 25 years we have implemented no
fewer than four major versions of GHC's build system; it improved each
time, but the result is still manifestly inadequate.

\item We describe Shake, an embedded domain specfic language (or library)
in Haskell that directly addresses the challenges we identify (Section~\ref{sec:shake}).
...
two decades (Section~\ref{sec:solutions}).
...
in Section~\ref{sec:abstractions} an overview of the new build system we have
built for GHC, and the new abstractions (not part of Shake) that we built
to support it.
...
common use-cases, as we disuss in Section~\ref{sec:ghc}.
\end{itemize}
None of this is fundamentally new; we review related work in
Section~\ref{section-review}.
